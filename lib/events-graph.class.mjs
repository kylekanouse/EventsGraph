...
class EventsGraph {
  ...
  constructor(id, ele, graphData = {}, events = [], users = []) {
    ...
    this._monitor = new EventsGraphMonitor();
  ...
  _addNodeFromGraph(node) {
    this._nodes.push( new EventsGraphNode(node) );
  ...
  _onGraphFirstLoad() {
    ...
    const url = './assets/audio/explosion.ogg';
    ...
    this._graph
      ...
      .nodeThreeObject( (node) => {
        ...
        if (node.icon) {
          ...
          const sphere = new THREE.SphereGeometry(0.5 * node.val,20,20);
