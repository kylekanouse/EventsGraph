...
class EventsGraph {
  ...
  _buildLinksFromGraphData(linksData) {
    return linksData.map((linkData) => {
      return new EventsGraphLink(linkData);
    });
  }
  ...
  _onGraphFirstLoad() {
    ...
    audioLoader.load( url, function( buffer ) {
      ...
      sound.setLoop( false );
      sound.setVolume( 1 );
      console.log('_onGraphRendered() | PLAY ==========> url = ', url);
      sound.play();
    });
  }
  ...
  sendEvent(event) {
    ...
    event.playSound(this._listener);
