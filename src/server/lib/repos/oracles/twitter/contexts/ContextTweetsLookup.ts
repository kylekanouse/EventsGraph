...
class ContextTweetsLookup extends Context {
  ...
  protected _id: string = constants.TWITTER_TWEETS_LOOKUP_CONTEXT_ID
  ...
  public async getData(request: IEventsGraphCollectionContextRequest) : Promise<IEventsGraphCollectionContextResponse> {
    return TweetService.getTweets(constants.TWITTER_TWEET_PATH_ID, this._buildRequestParameters(request), request)
  }
...
// Export
export default new ContextTweetsLookup()